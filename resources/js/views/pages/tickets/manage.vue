    ...
    methods: {
        connectAndSetupSocket() {
            ...
            this.socket.on('connect', () => {
                ...
                this.socket.emit('joinRoom', this.ticketId);
            });

            // Listening for admin updates
            this.socket.on('updateTicketAdmin', (data) => {
            console.log('Update from admin:', data.message);

            this.getTicketDisply();
            // Handle the admin update, e.g., update the ticket replies
            });

            // // Listening for customer updates
            this.socket.on('updateTicketCustomer', (data) => {
                this.getTicketDisply();
            // Handle the customer update
            });
        },
        sendReply() {
            const data = {
            ...
            this.socket.emit('customerReply', data); // Or 'customerReply' for the customer side
        },
        getTicketDisply() {
            const self = this;
        ...
        addReply() {
            ...
            axios.post('api/tickets/' + self.$route.params.uuid + '/reply', self.ticketReply).then(function (response) {
                ...
                //  this.ticket.ticketReplies.push(response.data.newReply);
